...
export const paidEnrollment = async (req, res) => {
  try {
    ...
    const product = await stripe.products.create({
      ...
      description: course.description, // description of the course
      metadata: {
...
//     // calculate the price of the course
//     const coursePrice = course.price;
//     const applicationFee = (coursePrice * 30) / 100;
//     const instructorFee = coursePrice - applicationFee;
...
export const stripeSuccess = async (req, res) => {
  try {
    const course = await Course.findById(req.params.courseId).exec();     // find course

    const user = await User.findById(req.auth._id).exec();     // get user from db to get stripe session id

    if (!user.stripeSession.id) return res.sendStatus(400);    // if no stripe session return
...
export const userCourses = async (req, res) => {
  ...
  res.json(courses);
    ...
    console.log("USER COURSES ERR", err);
    res.json({ success: false });
  }
};
